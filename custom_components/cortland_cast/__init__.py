...
from .const import DOMAIN, CONF_HOST, CONF_PORT, CONF_SHOW_PANEL
from .panel import (
    async_register_panel,
    async_register_dedicated_panel,
    async_unregister_panel,
    async_unregister_dedicated_panel,
)
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    ...
    domain_data = hass.data.setdefault(DOMAIN, {})
    if not domain_data.get("_card_resource_registered", False):
        card_module_urls = [
            f"/{DOMAIN}-panel/cortland-cast-card.js",
            f"/{DOMAIN}-panel/cortland-cast-dedicated-panel.js",
        ]
        add_module = getattr(frontend, "async_add_extra_module_url", None)
        for card_module_url in card_module_urls:
            if add_module:
                add_module(hass, card_module_url)
            else:
                add_module_sync = getattr(frontend, "add_extra_module_url", None)
                if add_module_sync:
                    add_module_sync(hass, card_module_url)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Serve frontend assets for the Cortland Cast card and panel.
    await async_register_static_assets(hass)

    # Register the Lovelace card resources once per Home Assistant instance.
    if not hass.data.get(DOMAIN, {}).get("_card_resource_registered", False):
        card_module_urls = [
            f"/{DOMAIN}-panel/cortland-cast-card.js",
            f"/{DOMAIN}-panel/cortland-cast-dedicated-panel.js",
        ]
        ...
        for card_module_url in card_module_urls:
            if add_module:
                ...
                if add_module_sync:
                    ...
                    _LOGGER.warning(
                        "Home Assistant frontend does not support auto-registering module resources; "
    ...
    hass.data[DOMAIN].setdefault("_dedicated_panel_registered", False)
...
async def _async_update_panel_for_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    if show_panel:
        ...
        if not domain_data.get("_dedicated_panel_registered"):
            ...
            dedicated_registered = await async_register_dedicated_panel(hass, server_url=server_url)
            if dedicated_registered:
                domain_data["_dedicated_panel_registered"] = True
    else:
        ...
        if not panel_entries and domain_data.get("_dedicated_panel_registered"):
            await async_unregister_dedicated_panel(hass)
            domain_data["_dedicated_panel_registered"] = False
