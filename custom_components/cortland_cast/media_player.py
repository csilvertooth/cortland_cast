...
_LOGGER.debug("LOADED cortland_cast/media_player.py VERSION 0.9.0")

# Note: Playlist and Artist artwork uses default icons when no artwork found.
# Server returns blank PNG placeholders. Future versions will support custom artwork.
...
async def async_setup_entry(
    ...
    hass.data[DOMAIN][entry_id]["player_ref"] = player
    hass.data[DOMAIN][entry_id]["config_entry"] = config_entry
    hass.data[DOMAIN][entry_id]["airplay_devices"] = {}

    # Configure debug logging if enabled
    debug_logging = config_entry.options.get(CONF_DEBUG_LOGGING, False)
    if debug_logging:
        _LOGGER.setLevel(logging.DEBUG)
        _LOGGER.info("Debug logging enabled for Cortland Cast integration")
    else:
    ...
    hass.data[DOMAIN][entry_id]["add_entities"] = add_airplay_entities
...
async def _discover_airplay_devices(hass: HomeAssistant, player: CortlandCastController) -> None:
    ...
    entry_id = player._entry.entry_id
        ...
        try:
            ...
            async with session.get(f"{base_url}/airplay/devices") as resp:
                if resp.status == 200:
                    ...
                    _LOGGER.info("Found %s AirPlay devices from server for entry %s", len(devices), entry_id)
                    existing_ids = entry_data.get("airplay_devices", {})
                    new_entities = []
                    ...
                    for device in devices:
                        ...
                        if device_id in existing_ids:
                            ...
                            _LOGGER.info("Creating new AirPlay device entity: %s (%s) for entry %s", device_name, device_id, entry_id)
                    ...
                    if new_entities:
                        add_entities_func = entry_data.get("add_entities")
                        if add_entities_func:
                            ...
                            _LOGGER.error("add_entities function not found in hass.data for entry %s", entry_id)

                    entry_data["airplay_devices"] = existing_ids
            ...
            _LOGGER.error("AirPlay discovery error for entry %s: %s", entry_id, e, exc_info=True)
...
class CortlandCastController(MediaPlayerEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        # Fetch computer name for better entity naming - do this during setup
        self._computer_name = "Cortland Cast Controller"  # Default fallback

        # Set initial name, will be updated when computer name is fetched
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Expose controller metadata to the frontend."""
        attrs: dict[str, Any] = {
            "cortland_cast_role": "controller",
            "cortland_cast_entry_id": self._entry.entry_id,
        }
        if self._computer_name:
            attrs["computer_name"] = self._computer_name
        return attrs

    async def async_added_to_hass(self) -> None:
        """Load initial state and start WebSocket."""
        # Fetch computer name for entity naming
        try:
            async with self._session.get(f"{self._base_url}/computer_name", timeout=ClientTimeout(total=5)) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    computer_name = data.get('computer_name', 'Cortland Cast Server')
                    self._computer_name = f"Music on {computer_name}"
                    self._attr_name = self._computer_name
                    _LOGGER.info(f"Set entity name to: {self._computer_name}")
                else:
                    self._computer_name = "Cortland Cast Controller"
                    self._attr_name = self._computer_name
        except Exception as e:
            _LOGGER.warning(f"Failed to get computer name: {e}")
            self._computer_name = "Cortland Cast Controller"
            self._attr_name = self._computer_name
        ...
        try:
            ...
            if data:
                ...
                if event_type == "ping":
                    ...
                    inner_data = data.get("data", {})
                    if "enabled" in inner_data:
                        self._attr_shuffle = bool(inner_data.get("enabled"))
                        _LOGGER.debug("Shuffle updated via WebSocket: %s", self._attr_shuffle)
                    self.async_write_ha_state()
                ...
                # Removed position updates during playback to prevent seeking issues
                    ...
                    _LOGGER.debug("WS airplay_devices update: %s devices for entry %s", len(devices), self._entry.entry_id)

                    # Update AirPlay device entities - per entry
                    entry_data = self.hass.data.get(DOMAIN, {}).get(self._entry.entry_id, {})
                    airplay_devices = entry_data.get("airplay_devices", {})
                    active_entity_ids = []
    ...
    def _update_from_now_playing(self, data: dict, include_position: bool = False) -> None:
        ...
        # ✅ Tell HA this is “music” so UI shows artist/album reliably
        if self._attr_media_title or self._attr_media_artist or self._attr_media_album_name:
            self._attr_media_content_type = MediaType.MUSIC
            # Strongly recommended: stable-ish ID (or use a real track id if you have one)
            self._attr_media_content_id = (
                data.get("id")
                or data.get("persistent_id")
                or f"{self._attr_media_artist or ''}|{self._attr_media_album_name or ''}|{self._attr_media_title or ''}"
            )
        else:
    ...
    async def async_browse_media(self, media_content_type: str | None = None, media_content_id: str | None = None) -> BrowseMedia:
        ...
        if media_content_id == "playlists":
                ...
                async with self._session.get(f"{self._base_url}/{media_content_id}", timeout=ClientTimeout(total=30)) as resp:
                    ...
                    if resp.status == 200:
                        ...
                        for item in (items or []):
                            name = item
                            # Add thumbnail for playlists
                            import urllib.parse
                            encoded_name = urllib.parse.quote(name)
                            thumbnail = f"{self._base_url}/playlist_artwork?name={encoded_name}"
            ...
            # Group items by first letter, ignoring "The " prefix for grouping
            letter_groups = {}
            for item in items:
                name = item.strip()
                if name:
                    # Remove leading "The" for grouping purposes, but keep original name for display
                    grouping_name = name.lower()
                    if grouping_name.startswith('the '):
                        grouping_name = grouping_name[4:]  # Remove "the " prefix

                    first_char = grouping_name[0].upper()
        ...
        if media_content_id and ':' in media_content_id:
            ...
            if len(parts) == 2:
                ...
                if category in ["playlists", "albums", "artists"]:
                    ...
                    # Filter items for this letter using the same grouping logic
                    filtered_items = []
                    is_numeric = letter == '#'
                    for item in items:
                        name = item.strip()
                        if name:
                            # Use the same logic as grouping to determine which letter this item belongs to
                            grouping_name = name.lower()
                            if grouping_name.startswith('the '):
                                grouping_name = grouping_name[4:]  # Remove "the " prefix
                    ...
                    # Sort filtered items, ignoring leading articles (The, A, An)
                    def sort_key(item):
                        """Sort key that ignores leading articles."""
                        # Convert to lowercase for case-insensitive comparison
                        item_lower = item.strip().lower()
                        # Remove leading 'the' article
                        if item_lower.startswith('the '):
                            return item_lower[4:]
                        return item_lower
                    ...
                    for name in filtered_items:
                        ...
                        # Add thumbnails for albums, artists, and playlists
                        import urllib.parse
                        encoded_name = urllib.parse.quote(name)
                        ...
                        if category == "albums":
                            ...
                            thumbnail = f"{self._base_url}/artist_artwork?name={encoded_name}"
            ...
            try:
                async with self._session.get(f"{self._base_url}/playlist_tracks", params={"name": playlist_name}, timeout=ClientTimeout(total=30)) as resp:
                    if resp.status == 200:
                        ...
                        for idx, track in enumerate(tracks or [], start=1):
            ...
            try:
                async with self._session.get(f"{self._base_url}/album_tracks", params={"name": album_name}, timeout=ClientTimeout(total=30)) as resp:
                    if resp.status == 200:
                        ...
                        _LOGGER.debug("Album %s: %s tracks, data type: %s", album_name,len(tracks), type(tracks))
                        if tracks:
                            _LOGGER.debug("First track sample: %s", tracks[0] if tracks else "none")
                        
                        for track in (tracks or []):
                            # Handle new format with track numbers from Apple Music
                            name = None
                            title = None
                            ...
                            if isinstance(track, dict):
                                ...
                                name = track.get('name', 'Unknown')
                                # Use actual track number from Apple Music, or omit if 0
                                if track_num > 0:
                                    title = f"{track_num}. {name}"
                                else:
                                    title = name
                            else:
                ...
                async with self._session.get(f"{self._base_url}/artist_albums", params={"name": artist_name}, timeout=ClientTimeout(total=30)) as resp:
                    if resp.status == 200:
                        ...
                        for album in (albums or []):
                            name = album
                            # Add album artwork for artist albums
                            import urllib.parse
                            encoded_name = urllib.parse.quote(name)
                            thumbnail = f"{self._base_url}/album_artwork?name={encoded_name}"
                            children.append(BrowseMedia(
        ...
        # Apple Music Catalog - top level menu
        if media_content_id == "catalog":
            return BrowseMedia(
                title="Apple Music",
                media_class=MediaClass.DIRECTORY,
                media_content_id="catalog",
                ...
                children=[
                    BrowseMedia(
                        ...
                        media_class=MediaClass.DIRECTORY,
                        media_content_id="catalog:search",
                        media_content_type=MediaType.MUSIC,
                        can_play=False,
                        can_expand=True,
                    ),
                    BrowseMedia(
                        title="Top Songs",
                        media_class=MediaClass.DIRECTORY,
                        media_content_id="catalog:charts:songs",
                        media_content_type=MediaType.MUSIC,
                        can_play=False,
                        can_expand=True,
                    ),
                    BrowseMedia(
                        title="Top Albums",
                        media_class=MediaClass.DIRECTORY,
                        media_content_id="catalog:charts:albums",
                        media_content_type=MediaType.MUSIC,
                        can_play=False,
                        can_expand=True,
                    ),
                    BrowseMedia(
                        title="Top Playlists",
                        media_class=MediaClass.DIRECTORY,
                        media_content_id="catalog:charts:playlists",
                        media_content_type=MediaType.MUSIC,
                        can_play=False,
                        can_expand=True,
                    ),
                    BrowseMedia(
                        title="For You",
                        media_class=MediaClass.DIRECTORY,
                        media_content_id="catalog:recommendations",
                        media_content_type=MediaType.MUSIC,
                        can_play=False,
                        can_expand=True,
                    ),
        ...
        # Apple Music Catalog - Charts
        if media_content_id and media_content_id.startswith("catalog:charts:"):
            chart_type = media_content_id.split(":")[-1]  # songs, albums, or playlists
            children = []
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/charts",
                        ...
                        if chart_type == "songs":
                            for item in data.get("songs", []):
                                children.append(BrowseMedia(
                                    ...
                                    media_class=MediaClass.TRACK,
                                    media_content_id=f"catalog_song:{item['id']}",
                                    media_content_type=MediaType.MUSIC,
                                    can_play=True,
                                    can_expand=False,
                            ...
                            for item in data.get("albums", []):
                                children.append(BrowseMedia(
                                    title=f"{item['title']} - {item['artist']}",
                                    media_class=MediaClass.ALBUM,
                                    media_content_id=f"catalog_album:{item['id']}",
                                    media_content_type=MediaType.ALBUM,
                                    can_play=True,
                                    can_expand=True,
                                    thumbnail=item.get("artwork_url"),
                            ...
                            for item in data.get("playlists", []):
                                children.append(BrowseMedia(
                                    title=item["name"],
                                    media_class=MediaClass.PLAYLIST,
                                    media_content_id=f"catalog_playlist:{item['id']}",
                                    media_content_type=MediaType.PLAYLIST,
                                    can_play=True,
                                    can_expand=True,
                                    thumbnail=item.get("artwork_url"),
                ...
                _LOGGER.warning("Catalog charts fetch failed: %s", e)

            title_map = {"songs": "Top Songs", "albums": "Top Albums", "playlists": "Top Playlists"}
            return BrowseMedia(
                title=title_map.get(chart_type, "Charts"),
                media_class=MediaClass.DIRECTORY,
                media_content_id=media_content_id,
                media_content_type=MediaType.MUSIC,
                can_play=False,
        ...
        # Apple Music Catalog - Recommendations
        if media_content_id == "catalog:recommendations":
            children = []
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/recommendations",
                    timeout=ClientTimeout(total=30),
                ) as resp:
                    if resp.status == 200:
                        groups = await resp.json()
                        for group in (groups or []):
                            for item in group.get("albums", []):
                                children.append(BrowseMedia(
                                    title=f"{item['title']} - {item['artist']}",
                                    media_class=MediaClass.ALBUM,
                                    media_content_id=f"catalog_album:{item['id']}",
                            ...
                            for item in group.get("playlists", []):
                                children.append(BrowseMedia(
                                    title=item["name"],
                                    media_class=MediaClass.PLAYLIST,
                                    media_content_id=f"catalog_playlist:{item['id']}",
                ...
                _LOGGER.warning("Catalog recommendations fetch failed: %s", e)
        ...
        # Apple Music Catalog - Search (using query from media_content_id)
        if media_content_id and media_content_id.startswith("catalog:search:"):
            import urllib.parse
            query = media_content_id.split(":", 2)[2]
            children = []
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/search",
                    ...
                    if resp.status == 200:
                        ...
                        for item in data.get("artists", []):
                            children.append(BrowseMedia(
                                title=item["name"],
                                media_class=MediaClass.ARTIST,
                                media_content_id=f"catalog_artist:{item['id']}",
                                media_content_type=MediaType.ARTIST,
                                can_play=False,
                                can_expand=True,
                                thumbnail=item.get("artwork_url"),
                ...
                _LOGGER.warning("Catalog search failed: %s", e)
        ...
        # Apple Music Catalog - Album detail
        if media_content_id and media_content_id.startswith("catalog_album:"):
            album_id = media_content_id.split(":", 1)[1]
            children = []
            album_title = "Album"
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/album/{album_id}",
                    timeout=ClientTimeout(total=30),
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        ...
                        for track in data.get("tracks", []):
                            ...
                            prefix = f"{track_num}. " if track_num else ""
                            children.append(BrowseMedia(
                                title=f"{prefix}{track['title']}",
                                media_class=MediaClass.TRACK,
                                media_content_id=f"catalog_song:{track['id']}",
                                media_content_type=MediaType.MUSIC,
                                can_play=True,
                                can_expand=False,
                ...
                _LOGGER.warning("Catalog album detail fetch failed: %s", e)
        ...
        # Apple Music Catalog - Playlist detail
        if media_content_id and media_content_id.startswith("catalog_playlist:"):
            playlist_id = media_content_id.split(":", 1)[1]
            children = []
            playlist_name = "Playlist"
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/playlist/{playlist_id}",
                    timeout=ClientTimeout(total=30),
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                ...
                _LOGGER.warning("Catalog playlist detail fetch failed: %s", e)
        ...
        # Apple Music Catalog - Artist detail
        if media_content_id and media_content_id.startswith("catalog_artist:"):
            artist_id = media_content_id.split(":", 1)[1]
            children = []
            artist_name = "Artist"
            try:
                async with self._session.get(
                    f"{self._base_url}/catalog/artist/{artist_id}",
                    timeout=ClientTimeout(total=30),
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        artist_name = data.get("name", "Artist")
                        # Add top songs
                        for track in data.get("top_songs", []):
                            children.append(BrowseMedia(
                                title=f"{track['title']}",
                                media_class=MediaClass.TRACK,
                                media_content_id=f"catalog_song:{track['id']}",
                        ...
                        for album in data.get("albums", []):
                            children.append(BrowseMedia(
                                title=album["title"],
                                media_class=MediaClass.ALBUM,
                                media_content_id=f"catalog_album:{album['id']}",
                                media_content_type=MediaType.ALBUM,
                                can_play=True,
                                can_expand=True,
                                thumbnail=album.get("artwork_url"),
                            ))
            except Exception as e:
                _LOGGER.warning("Catalog artist detail fetch failed: %s", e)
    ...
    async def async_play_media(self, media_type: str, media_id: str, **kwargs) -> None:
        ...
        # Handle Apple Music catalog content
        if media_id.startswith("catalog_song:"):
            store_id = media_id.split(":", 1)[1]
            _LOGGER.debug("Catalog song request: store_id=%s", store_id)
            media_data = {"type": "catalog_song", "store_id": store_id}
            try:
                async with self._session.post(
                    f"{self._base_url}/play", json=media_data
                ) as response:
                    if response.status == 200:
                        self._state = MediaPlayerState.PLAYING
                        self.async_write_ha_state()
            except Exception as e:
                _LOGGER.debug("Catalog song play failed: %s", e)
            return

        if media_id.startswith("catalog_album:"):
            store_id = media_id.split(":", 1)[1]
            _LOGGER.debug("Catalog album request: store_id=%s", store_id)
            media_data = {"type": "catalog_album", "store_id": store_id}
            try:
                async with self._session.post(
                    f"{self._base_url}/play", json=media_data
                ) as response:
                ...
                _LOGGER.debug("Catalog album play failed: %s", e)
            return

        if media_id.startswith("catalog_playlist:"):
            store_id = media_id.split(":", 1)[1]
            _LOGGER.debug("Catalog playlist request: store_id=%s", store_id)
            media_data = {"type": "catalog_playlist", "store_id": store_id}
            try:
                async with self._session.post(
                    f"{self._base_url}/play", json=media_data
                ) as response:
                ...
                _LOGGER.debug("Catalog playlist play failed: %s", e)
            return
        ...
        if ":" in media_id:
            ...
            if media_prefix == "track" and len(parts) == 4 and parts[1] == "playlist":
                ...
                # Play the specific track from the playlist
                media_data = {"type": "track", "playlist": playlist_name, "name": track_name}
    ...
    async def async_join_players(self, group_members: list[str]) -> None:
        """Join AirPlay devices to this player (group mode) - only allows devices from this controller."""
        try:
            _LOGGER.info("Join players called with group_members: %s for entry %s", group_members, self._entry.entry_id)

            # Validate that all group_members belong to this controller's AirPlay devices
            entry_data = self.hass.data.get(DOMAIN, {}).get(self._entry.entry_id, {})
            airplay_devices = entry_data.get("airplay_devices", {})

            # Get all valid entity_ids for this controller's AirPlay devices
            valid_entity_ids = {entity.entity_id for entity in airplay_devices.values() if entity.entity_id}

            # Check if any requested entity is not from this controller
            invalid_entities = [eid for eid in group_members if eid not in valid_entity_ids]
            if invalid_entities:
                _LOGGER.error("Rejected join request for non-controller entities: %s (entry %s)", invalid_entities, self._entry.entry_id)
                raise ValueError(f"Cannot join entities that don't belong to this controller: {invalid_entities}")

            # Find devices that match the group_members entity IDs - per entry
            device_ids = []
            ...
            async with self._session.post(
                ...
                if response.status == 200:
                    # Identify devices that are being newly activated (not currently active)
                    newly_activated_device_ids = []
                    for device_id, entity in airplay_devices.items():
                        was_active = entity._is_active
                        should_be_active = device_id in device_ids
                        if should_be_active and not was_active:  # Newly activated
                            newly_activated_device_ids.append(device_id)

                    # Set volume to 20% for newly activated devices
                    for device_id in newly_activated_device_ids:
                        try:
                            _LOGGER.info("Setting volume to 20%% for newly joined device: %s", device_id)
                            async with self._session.post(
                                f"{self._base_url}/airplay/set_volume",
                                json={"device_id": device_id, "volume": 20}
                            ) as vol_response:
                                if vol_response.status == 200:
                                    _LOGGER.debug("Successfully set volume to 20%% for device %s", device_id)
                                else:
                                    _LOGGER.warning("Failed to set volume for device %s, status: %s", device_id, vol_response.status)
                        except Exception as e:
                            _LOGGER.error("Error setting volume for device %s: %s", device_id, e)
            ...
            _LOGGER.error("Failed to join AirPlay devices for entry %s: %s", self._entry.entry_id, e, exc_info=True)
            raise
    ...
    async def _update_group_members(self) -> None:
        ...
        try:
            ...
            self._attr_group_members = active_entity_ids
            self.async_write_ha_state()
            _LOGGER.debug("Updated group members to: %s for entry %s", active_entity_ids, self._entry.entry_id)
        except Exception as e:
            _LOGGER.error("Failed to update group members for entry %s: %s", self._entry.entry_id, e)
    ...
    async def _async_unjoin_player(self) -> None:
        ...
        try:
            _LOGGER.info("Unjoin player called for entry %s", self._entry.entry_id)
            ...
            async with self._session.post(
                ...
                if response.status == 200:
                    # Update all device states - per entry
                    entry_data = self.hass.data.get(DOMAIN, {}).get(self._entry.entry_id, {})
                    airplay_devices = entry_data.get("airplay_devices", {})
                    for device in airplay_devices.values():
                    ...
                    self._attr_group_members = []
                    self.async_write_ha_state()
                    _LOGGER.info("Successfully unjoined all AirPlay devices for entry %s", self._entry.entry_id)
                else:
                    response_text = await response.text()
                    _LOGGER.error("Unjoin failed, status %s: %s", response.status, response_text)
        except Exception as e:
            _LOGGER.error("Failed to unjoin AirPlay devices for entry %s: %s", self._entry.entry_id, e, exc_info=True)
            raise
