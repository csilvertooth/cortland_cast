...
class AppleMusicAirPlayDevice(MediaPlayerEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"apple_music_airplay_{device_id}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, f"airplay_{device_id}")},
            name=device_name,
            manufacturer="Apple",
        ...
        # Supported features - add GROUPING so devices appear in join interface
    ...
    def state(self) -> MediaPlayerState:
        ...
        if self._is_active:
            ...
            if main_player := self.hass.data.get(DOMAIN, {}).get("player_ref"):
        ...
        try:
            ...
            async with self._session.post(
                ...
                if response.status == 200:
                    ...
                    main_player = self.hass.data.get(DOMAIN, {}).get("player_ref")
                    if main_player:
