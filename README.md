...
![Cortland Cast Logo](images/cortland_cast_server_image.png)

## Summary

Cortland Cast bridges a dedicated macOS Apple Music controller with a Home Assistant integration so you can browse, search, and play your full Apple Music library (plus AirPlay speakers) from any Home Assistant dashboard. The Swift-based Cortland Cast Server talks to Apple Music through Scripting Bridge/AppleScript, exposes a REST + WebSocket API, maintains artwork caches (album, playlist, and artist), and provides local tools for managing your media library. The Home Assistant custom component pairs with that API to deliver a full media player experience, media browser, AirPlay grouping, power control, and diagnostics directly in Home Assistant.
...
- Volume slider, mute, and automatic volume safety: new AirPlay devices start at 20% and grouping adjustments only change volumes in 5% increments.
- Media Browser for playlists, artists, albums, and tracks with alphabetical grouping, proper “The/A” sorting, track numbers, playlist and artist artwork, and server-side search when browsing gets unwieldy.
...
- macOS 12 Monterey or later with the Apple Music app signed in to the desired library.
