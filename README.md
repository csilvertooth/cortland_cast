...
- Native Lovelace media player card (`custom:cortland-cast-card`) that bundles the main controller, AirPlay tiles, and quick actions into one dashboard widget.
...
### Lovelace Card
The integration automatically registers the Cortland Cast card resource. Add the card to a dashboard with:
...
entry_id: <config_entry_id>   # filters AirPlay tiles by entry
show_airplay: true
show_actions: true
```

If Home Assistant blocks auto-registered resources, add a manual Lovelace resource pointing to:
`/cortland_cast-panel/cortland-cast-card.js`
