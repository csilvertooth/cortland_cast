...
// A simple shared logger that writes to file and console with log rotation
class ServerLogger {
    ...
    private func rotateIfNeeded() {
        ...
        do {
            ...
            if let fileSize = attrs[.size] as? UInt64, fileSize > maxLogFileSize {
                let backupURL = logFileURL.deletingLastPathComponent().appendingPathComponent("server.log.1")
                try? FileManager.default.removeItem(at: backupURL)
                try FileManager.default.moveItem(at: logFileURL, to: backupURL)
            }
        } catch {
            print("Failed to check/rotate log file: \(error)")
        }
    }
...
actor StateTracker {
    ...
    func hasChanged(_ key: String, newValue: Any) -> Bool {
        // Normalize all values to String for reliable comparison across Bool/Int/Double/NSNumber
        let newStr = String(describing: newValue)
        let oldStr = lastStates[key]

        // Always store the new value before returning
        lastStates[key] = newStr

        // First check for this key — treat as changed
    ...
    func updateState(_ key: String, value: Any) {
        lastStates[key] = String(describing: value)
    }
...
public class MusicServer {
    ...
    public func start(port: Int = 7766) async throws {
        ...
        // Ensure directories exist and clean up old cache files
    ...
    private func routes(_ app: Application) throws {
        // CORS middleware for panel direct API access
        ...
        // Register Apple Music catalog routes (search, browse, charts, etc.)
        try catalogRoutes(app)
        ...
        app.webSocket("ws") { req, ws in
            ...
            let monitorTask = monitorMusicState { change in
                let eventData = "data: {\"type\": \"\(change.type)\", \"data\": \(change.json)}\n"
                ws.send(eventData)
            }

            // Cancel monitoring when WebSocket disconnects
            ws.onClose.whenComplete { _ in
                monitorTask.cancel()
                serverLog("WebSocket disconnected, monitor task cancelled", level: "INFO")
            }
        }
        ...
        app.post("play") { req async throws -> [String: String] in
            ...
            // Handle Apple Music catalog playback (no name needed, uses store_id/store_url)
            if let mediaType = playReq.type, ["catalog_song", "catalog_album", "catalog_playlist"].contains(mediaType) {
                if let storeId = playReq.store_id {
                    let result = await MusicKitService.shared.playCatalogItem(type: mediaType, storeId: storeId)
                    print("Catalog play result: success=\(result.success), output='\(result.output)'")
                    return ["status": result.success ? "playing" : "error"]
                } else if let storeUrl = playReq.store_url {
                    let result = await MusicKitService.shared.playCatalogUrl(storeUrl)
                    print("Catalog URL play result: success=\(result.success), output='\(result.output)'")
                    return ["status": result.success ? "playing" : "error"]
                } else {
                    return ["status": "error", "message": "store_id or store_url required for catalog playback"]
                }
            }
    ...
    private func cleanupArtworkCache() {
        let maxCacheAgeDays: TimeInterval = 30
        let maxCacheSizeBytes: UInt64 = 500 * 1024 * 1024 // 500MB

        let categories = ["artwork", "artist_profiles", "playlist_artwork"]
        let cutoffDate = Date().addingTimeInterval(-maxCacheAgeDays * 24 * 60 * 60)
        ...
        for category in categories {
            ...
            let sorted = files.compactMap { url -> (URL, Date, UInt64)? in
                ...
                return (url, modDate, UInt64(size))
            }.sorted { $0.1 < $1.1 } // oldest first

            var totalSize: UInt64 = sorted.reduce(0) { $0 + $1.2 }
            var deletedCount = 0

            for (url, modDate, fileSize) in sorted {
                let shouldDelete = modDate < cutoffDate || totalSize > maxCacheSizeBytes
                if shouldDelete {
            ...
            if deletedCount > 0 {
                serverLog("Artwork cache cleanup (\(category)): deleted \(deletedCount) files", level: "INFO")
            }
        }
    }
...
// Consolidated AppleScript snapshot — returns state, position, and repeat in one osascript call
...
func getMusicStateSnapshot() -> MusicStateSnapshot? {
    let script = """
    tell application "Music"
        try
            ...
            return stateText & "|" & posText & "|" & repeatText
        on error errMsg
            return "ERROR|0|off"
        end try
    end tell
    ...
    let parts = result.output.trimmingCharacters(in: .whitespacesAndNewlines).split(separator: "|", maxSplits: 2)
    guard parts.count == 3 else { return nil }

    return MusicStateSnapshot(
        playerState: String(parts[0]),
        position: Double(parts[1]) ?? 0.0,
        repeatMode: String(parts[2])
    )
}

// Track when we last checked AirPlay devices
var lastAirPlayCheck: Date = .distantPast
...
func getNowPlayingInfo() -> [String: Any] {
    ...
    if let music = SBApplication(bundleIdentifier: "com.apple.Music") {
        // Get current track info via ScriptingBridge (no process spawn)
        ...
        // Get state + position + repeat in ONE osascript call (was 2-3 separate calls)
        if let snapshot = getMusicStateSnapshot() {
            nowPlaying["state"] = snapshot.playerState.lowercased()
            nowPlaying["position"] = snapshot.position
            nowPlaying["repeat"] = snapshot.repeatMode
            nowPlaying["is_playing"] = snapshot.playerState.lowercased().contains("play")
        }

        // Volume and shuffle via ScriptingBridge (no process spawn)
...
@discardableResult
func monitorMusicState(changeHandler: @escaping (StateChange) -> Void) -> Task<Void, Never> {
    let task = Task {
...
func getCurrentFullState() async -> [String: Any] {
    ...
    // Get state + position + repeat in ONE osascript call (was 3 separate calls)
    if let snapshot = getMusicStateSnapshot() {
        state["player_state"] = snapshot.playerState
        state["position"] = snapshot.position
        state["repeat_mode"] = snapshot.repeatMode
    }

    // Get shuffle/volume via ScriptingBridge (no process spawn)
    ...
    // Get current track via ScriptingBridge
    if let currentTrack = music.value(forKey: "currentTrack") as? NSObject {
...
func checkForStateChanges(changeHandler: ((StateChange) -> Void)? = nil) async {
    ...
    // --- Single AppleScript call for state + position + repeat (was 3 separate osascript processes) ---
    let snapshot = getMusicStateSnapshot()
    let currentState = snapshot?.playerState ?? "unknown"
    let position = snapshot?.position ?? 0.0
    let repeatMode = snapshot?.repeatMode ?? "off"

    // Check volume changes via ScriptingBridge (no process spawn)
    if let volume = music.value(forKey: "soundVolume") as? NSNumber {
        if await stateTracker.hasChanged("volume", newValue: volume.intValue) {
    ...
    // Check position updates (from consolidated snapshot)
    ...
    // Check for track changes via ScriptingBridge
    ...
    // Check shuffle via ScriptingBridge (no process spawn)
    if let shuffleNS = music.value(forKey: "shuffleEnabled") as? NSNumber,
    ...
    // Check repeat mode (from consolidated snapshot, no extra osascript)
    ...
    // Check AirPlay devices — throttled to every N seconds (uses pollDevicesMs setting)
    let settings = SettingsManager.shared.get()
    let deviceIntervalSeconds = Double(settings.pollDevicesMs) / 1000.0
    if Date().timeIntervalSince(lastAirPlayCheck) >= deviceIntervalSeconds {
        lastAirPlayCheck = Date()
        await checkAirPlayDevices(changeHandler: changeHandler)
    }
}

func checkAirPlayDevices(changeHandler: ((StateChange) -> Void)? = nil) async {
    let deviceScript = """
    tell application "Music"
        try
            set airplayDevices to {}
    ...
    if deviceResult.success && !deviceResult.output.hasPrefix("ERROR:") {
        ...
        for deviceStr in deviceStrings {
            ...
            for part in parts {
                ...
                if keyValue.count == 2 {
                    ...
                    let value = String(keyValue[1])
                    switch key {
                    case "id": deviceID = value
                    case "name": deviceName = value
                    case "active": deviceActive = (value.lowercased() == "true")
                    case "volume": deviceVolume = Int(value) ?? 0
                    case "kind": deviceKind = value
                    case "available": deviceAvailable = (value.lowercased() == "true")
                    default: break
                    }
                }
            }
            ...
            if !deviceID.isEmpty && !deviceName.isEmpty {
                devices.append([
                    ...
                    "active": deviceActive, "volume": deviceVolume,
                    "kind": deviceKind, "available": deviceAvailable
                ] as [String: Any])
            }
        }
        ...
        if await stateTracker.hasChanged("airplay_devices", newValue: deviceHash) {
            ...
            changeHandler?(change)
            serverLog("AirPlay devices changed: \(devices.count) devices", level: "INFO")
        }
    }
}
