...
struct ContentView: View {
    ...
    @State private var musicKitAuthorized = MusicAuthorization.currentStatus == .authorized
    @State private var musicKitSubscriptionActive = false
    @State private var isCheckingMusicKit = false
                ...
                HStack(alignment: .top, spacing: 20) {
                    ...
                    VStack(alignment: .center, spacing: 10) {
                        ...
                        VStack(alignment: .center, spacing: 8) {
                            ...
                            Text("Apple Music Catalog")
                                .font(.subheadline)
                                .foregroundColor(.secondary)

                            HStack(spacing: 6) {
                                Circle()
                                    .fill(musicKitAuthorized ? Color.green : Color.orange)
                                    .frame(width: 8, height: 8)
                                Text(musicKitAuthorized ? "MusicKit Authorized" : "MusicKit Not Authorized")
                                    .font(.caption)
                                    .foregroundColor(musicKitAuthorized ? .green : .orange)
                            }

                            if musicKitAuthorized {
                                HStack(spacing: 6) {
                                    Circle()
                                        .fill(musicKitSubscriptionActive ? Color.green : Color.orange)
                                        .frame(width: 8, height: 8)
                                    Text(musicKitSubscriptionActive ? "Subscription Active" : "No Subscription")
                                        .font(.caption)
                                        .foregroundColor(musicKitSubscriptionActive ? .green : .orange)
                                }
                            }
                            ...
                            if !musicKitAuthorized {
                                Button(action: {
                                    Task {
                                        isCheckingMusicKit = true
                                        let status = await MusicKitService.shared.requestAuthorization()
                                        musicKitAuthorized = status == .authorized
                                        if musicKitAuthorized {
                                            musicKitSubscriptionActive = await MusicKitService.shared.checkSubscription()
                                        }
                                        isCheckingMusicKit = false
                                    }
                                }) {
                                    if isCheckingMusicKit {
                                        HStack(spacing: 5) {
                                            ProgressView()
                                                .controlSize(.small)
                                            Text("Authorizing...")
                                ...
                                .buttonStyle(.bordered)
                                .disabled(isCheckingMusicKit)
                            }
                        }
                    }
                    .frame(width: geometry.size.width / 3 - 13)
                    .onAppear {
                        // Check MusicKit status on appear
                        Task {
