...
# Copy entitlements
if [ -f "CortlandCastServer.entitlements" ]; then
    cp CortlandCastServer.entitlements "Releases/Cortland Cast Server.app/Contents/"
    echo -e "${BLUE}Copied entitlements${NC}"
fi

# Code sign the app bundle (required for MusicKit)
SIGNING_IDENTITY="${CODESIGN_IDENTITY:--}"
echo -e "${BLUE}Signing app bundle (identity: $SIGNING_IDENTITY)...${NC}"
if [ -f "CortlandCastServer.entitlements" ]; then
    codesign --force --sign "$SIGNING_IDENTITY" \
        --entitlements CortlandCastServer.entitlements \
        --deep \
        "Releases/Cortland Cast Server.app"
else
    codesign --force --sign "$SIGNING_IDENTITY" \
        --deep \
        "Releases/Cortland Cast Server.app"
...
echo ""
echo -e "${YELLOW}To sign with a Developer ID, set CODESIGN_IDENTITY:${NC}"
echo -e "  CODESIGN_IDENTITY='Developer ID Application: Your Name' ./build-release.sh"
